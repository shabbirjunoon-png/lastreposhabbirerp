...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _backupLocal() async {
    ...
    try {
      ...
      _snack('Backup file ready — choose where to save or share');
    } catch (e) {
  ...
  Future<void> _backupDrive() async {
    if (!firebaseReady) {
      _snack(
        'Google Drive backup requires Firebase setup — see FIREBASE_SETUP.md',
        error: true,
      );
      return;
    }
    ...
    try {
      await BackupService.instance.backupToGoogleDrive();
      _snack('Backed up to Google Drive successfully');
    } catch (e) {
      _snack(
        'Drive backup failed: ${e.toString().replaceAll("Exception:", "").trim()}',
        error: true,
      );
    } finally {
      if (mounted) setState(() => _loadingBackupDrive = false);
    }
  }
  ...
  Future<void> _restoreDrive() async {
    if (!firebaseReady) {
      _snack(
        'Google Drive restore requires Firebase setup — see FIREBASE_SETUP.md',
        error: true,
      );
      return;
    }
    ...
    try {
      ...
      _snack('Data restored from Google Drive');
    } catch (e) {
      _snack(
      ...
      if (mounted) setState(() => _loadingRestoreDrive = false);
    }
  }

  Future<void> _logout() async {
    if (!firebaseReady) {
      _snack('Running in offline mode — no account to sign out of.');
      return;
    }
    final confirm =
        await showDialog<bool>(
  ...
  void _showAboutDialog(BuildContext context) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(
          'Powered by Shabbir Ahmed.\n\nThis app is totally AI-generated.',
          style: GoogleFonts.inter(fontSize: 14, height: 1.5),
        ),
        actions: [
          TextButton(
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          const AppHeader(
            title: 'Settings',
            subtitle: 'Account, security & data',
          ),
          Expanded(
            child: ListView(
              ...
              children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                'All data is saved locally. Complete FIREBASE_SETUP.md to enable cloud sync.',
                                style: GoogleFonts.inter(
                                  fontSize: 12,
                ...
                _Tile(
                  ...
                  subtitle: _patternEnabled
                      ? 'App locks when opened — tap to disable'
                      : 'Disabled — tap to enable',
                  trailing: Switch(
                    value: _patternEnabled,
                    onChanged: (_) => _togglePattern(),
                ...
                if (_patternEnabled)
                  _Tile(
                    ...
                    subtitle: 'Draw a new unlock pattern',
                ...
                _Tile(
                  ...
                  subtitle: 'Save a .json file to your phone storage',
                  loading: _loadingBackupLocal,
                  onTap: _backupLocal,
                ),
                _Tile(
                  ...
                  subtitle: isOffline
                      ? 'Requires Firebase setup (FIREBASE_SETUP.md)'
                      : 'Upload data to your Google Drive folder "ShabbirERP"',
                  loading: _loadingBackupDrive,
                  onTap: _backupDrive,
                  dimmed: isOffline,
                ),
                _Tile(
                  ...
                  subtitle: isOffline
                      ? 'Requires Firebase setup (FIREBASE_SETUP.md)'
                      : 'Download & replace all data from the last Drive backup',
                  loading: _loadingRestoreDrive,
                  onTap: _restoreDrive,
                  dimmed: isOffline,
                ),
                const SizedBox(height: 24),
                ...
                _Tile(
                  icon: Icons.logout,
                  title: isOffline ? 'Offline Mode' : 'Sign Out',
                  subtitle: isOffline
                      ? 'No account — complete Firebase setup to enable login'
                ...
                _Tile(
                  ...
                  title: 'About Shabbir ERP',
                  subtitle:
                      'Powered by Shabbir Ahmed. This app is totally AI-generated.',
  ...
  Widget _avatarLetter(String name, bool isOffline) {
    return Center(
      child: Icon(
        isOffline ? Icons.offline_bolt_outlined : Icons.person,
        color: isOffline ? AppColors.mutedForeground : AppColors.accent,
        size: 26,
      ),
    );
  }
